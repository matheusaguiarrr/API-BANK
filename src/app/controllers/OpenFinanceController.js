...
class OpenFinanceController {
  async store(req, res) {
    try {
      ...
      const existingOpenFinance = await OpenFinance.findOne({
        where: { account_id: account.id },
      });

      if (existingOpenFinance) {
        existingOpenFinance.status = authorization;
        existingOpenFinance.expiration = expiration;
        existingOpenFinance.expiration_date = expiration
          ? expirationDate
          : null;

        await existingOpenFinance.save();
  ...
  async show(req, res) {
    try {
      ...
      if (!openFinanceData) {
        return res
          .status(404)
          .json({ error: "Compartilhamento não encontrado" });
      }
      if (!openFinanceData.status) {
        return res
          .status(403)
          .json({ error: "Compartilhamento não autorizado" });
      }
      if (
